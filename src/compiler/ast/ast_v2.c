...
curium_ast_v2_node_t* curium_ast_v2_new(curium_ast_v2_kind_t kind, size_t line, size_t col) {
    curium_ast_v2_node_t* node = (curium_ast_v2_node_t*)curium_alloc(sizeof(curium_ast_v2_node_t), "ast_node");
    if (!node) return NULL;
...
static void curium_ast_v2_free_node_recursive(curium_ast_v2_node_t* node) {
    ...
    switch (node->kind) {
        ...
        case CURIUM_AST_V2_DYN_OP:
            ...
            curium_ast_v2_free_node_recursive(node->as.dyn_op.fallback);
            break;
